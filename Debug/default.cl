...
/*float2 csin(float2 f)
{
	...
	r = eiz - cinv(eiz);
	//r = cdiv(r, (float2)(0, 2));
	r = (float2)(r.y/2, -r.x/2);
	return r;
}*/
float2 csin(float2 f)
{
	float cx = cos(f.x), sx = sin(f.x);
	float ey = exp(f.y), e_y = 1/ey;
	return (float2)((e_y+ey)*sx/2, (ey-e_y)*cx/2);
}
...
// РџРѕРІРѕСЂРѕС‚ РІ СЂР°РґРёР°РЅР°С…
...
float2 Fracs(float2 z, float2 C, __global float2*c, int frac, int k)
{
	...
	case 3:
		return cmul(z, cexp(z))*c[1].y + cmul(z, clog(z))*c[2].x;
	case 4:
		return z - (2+c[2].y/10)*cdiv(cpow2(cpow2(z))-fx(1+c[3].x/3.5f),4*cmul(cpow2(z),z) - fx(c[1].x)) ; 
	case 5:
		return z - (2-c[0].y)*cdiv(ccube(z), 3*cmul(z,z));
	case 6: 
		return z - cpow2((0.7f + 2*c[0].x)*cdiv(ccube(z)-fx(1), 3*cpow2(z)));
	case 7:
	...
	case 13:
		return cacos(z+re(c[2]));
	case 14:
		return z - cexp(z+fx(c[0].x));
	case 15:
		return fx(c[0].y) + cmul(re(c[1]),z) + cmul(re(c[2]),cpow2(z)) 
		+ cmul(re(c[3]),cmul(z,cpow2(z)));
	case 16:
		return z - csin(z-re(c[2]/2));
	case 17:
		return z - cch(z+fx(c[1].y*c[0].x));
	case 18:
		return z - cmul(csin(z+fx(fast_length(c[1]))), fx(c[0].y+c[2].x)) + C + fx(c[1].x);
	case 19:
		return cexp(cmul(-z,z))+ fx(c[3].y);
	case 20:
		return (z+fx(c[0].y))*c[0].x + cdiv(z, C - fx(c[2].y))*c[2].x + cpow2(z)*c[3].x;
	case 21:
	...
	case 23:
		return cmul(csin(z+fx(c[2].x)), ccos(z+fx(c[1].y)));
	case 24:  
		return c[0].x*cexp(cmul(z,z)) + c[1].y*cexp(z) + C + c[2].x*z + c[3].y*cpow2(z);
	case 25:
		return cexp(cmul(-z,z+fx(c[0].y)))/sqrt2PI + C;
	case 26:
		return z - cdiv(cexp(cmul(-z,z)) + fx(c[2].x), -2*cmul(z,cexp(cmul(-z,z-fx(c[0].x)))));
	case 27:
		return cpow2(z) - cdiv(cexp(cmul(-z,z))+fx(c[2].x), cmul((-2+c[1].x)*z,cexp(cmul(-z,z)))) + cpow2(C);
	case 28:
		return cexp(csin(z+re(c[0]))+ re(c[2]));
	case 29:
		{
			float2 nz = cmul(z,z);
			float x = nz.x, y = nz.y;
			return (float2)(3*x*x - 6*x*y - y*y, 3*x*x + 6*x*y - 3*y*y);
		}
	case 30:
		return cmul(cmul(c[0]-c[1]+c[2]-c[3],z),fx(1)-z);
	case 31:
		return z + cdiv(ccos(z+fx(c[1].x)), csin(z-fx(c[1].y)));
	case 32:
		return cch(z + fx(c[3].x))*c[0].y + csin(cmul(cmul(z, fx(c[1].x)), cexp(C - fx(c[1].y))));
	default:
	...
	__global uchar4* pix, float4 posData, int xPixels,struct data st, __global float2* c)
{
	...
	{
		C = (float2)(x,y);
		zn = (float2)(-c[4].y,-c[4].x);
		zn_1 = zn + (float2)(0.1f, 0);
	}
	else
	{
		zn = (float2)(x,y);
		C = (float2)(-c[4].y,-c[4].x);
	...
	do
	{
		...
		z = zn - (2.1f + c[0].y)*cdiv(cmul(fn, zn - zn_1), fn_1 - fn) + C; 
		//z = zn - cdiv(cmul(fn, zn - zn_1), fn_1 - fn) + C;
		//z = cmul(zn,csin(cinv(zn))) + C;
		//z = csin(zn) + C;
	...
	int H,S,V;
	//H = (col*t*360.0f*(1+c[0].y))/st.iter+160-c[2].y*30;
	H = 0;
	S = 100*t/st.iter; 
	V = 100 + c[0].x*50*0;
	H = H % 360;
	...
	int r, g, b;
	r = (int)(255*R + c[0].x*100*col);
	g = (int)(255*G + c[1].x*100*col);
	b = (int)(255*B + c[2].x*100*col);

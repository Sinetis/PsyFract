	...
	__global uchar4* pix, float4 posData, int xPixels,struct data st, __global float2* c)
{
	...
	float color;
	if (it < st.iter)
		color = it*2.0f + col*7*c[0].x;// + st.k;
		//color = fast_length(cmul(z,zn))*20.0f + col*7*c[0].x;// + st.k;
	else
		color = col*7*c[0].x;// + st.k;
	uchar r,g,b;
	r = (uchar)((127 * cos(color/17.0f+col*4*c[1].x)) + 128);
	g = (uchar)((127 * cos(color/11.0f+col*4*c[2].x)) + 128);
	b = (uchar)((75 * cos(color/7.0f+col*4*c[3].x)) + 128);

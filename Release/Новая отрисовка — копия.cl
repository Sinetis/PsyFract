...
{
	return (float2)(f.x*f.x - f.y*f.y, 2*f.x*f.y);
}

float2 ccube(float2 f)
{
	return (float2)(f.x*(f.x*f.x - 3.0f*f.y*f.y), f.y*(3.0f*f.x*f.x - f.y*f.y));
}
...
float2 Simple(float2 z, float2 C, float2 c, int frac)
{
	c.x = c.x + c.y/10;
	switch(frac)
	{
		case 1:
			return c.x*cpow2(z) + C;
		case 2:
			return c.x*ccube(z) + C;
		case 3:
			return c.x*csin(z) + C;
		case 4:
			return c.x*ccos(z) + C;
		case 5:
			return c.x*cconj(z) + C;
		case 6:
			return cpow(z,c.x) + C;
		case 7:
			return c.x*cinv(z) + C;
		...
		case 9:
			return c.x*clog(z) + C;
		case 10:
			return c.x*csqrt(z) + C;
		case 11:
			return c.x*casin(z) + C;
		case 12:
			return c.x*cacos(z) + C;
		case 13:
			return c.x*cch(z) + C;
		case 14:
			return c.x*cpow2(z) + 1.0f;
		default:
			return c.x*z + C;
	}
}
	...
	__global uchar4* pix, float4 posData, int xPixels,struct data st, __global float2* c)
{
	...
	if (idx < xPixels/2)
		y = posData.x + posData.w*((int)idx + xPixels/4);
	else
		y = posData.x + posData.w*((int)idx - xPixels/4);
	...
	//st.k = 0;
	//st = changeData(st);
	float2 k = (float2)(cos(st.k), cos(st.k/2))*2.0f;
	/*c[0].x = sin(st.k*2);
	...
	if (idx < xPixels/2)
		...
		{
			t = it;
			fn1 = ccube(zn) + fx(1.0f);
			h = 0.001f;
			fn2 = 3*cpow2(zn + fx(h));
			//fn2 = Simple(zn, C, c[4], secondFrac(st));
			
			//z = zn - cdiv(fn2, fn1) + C;
			z = zn - cdiv(cmul(fn1, h), fn2 - fn1);
		...
		{
			t = it;
			fn1 = Simple(zn, C, 2, secondFrac(st));
			h = st.k;
			fn2 = ccos(zn + fx(h));
			//fn2 = Simple(zn, C, c[4], secondFrac(st));
	...
	int H,S,V;
	//H = 360*sin(it/30.0f)*col + st.col*40 + 180*cos(it/15.0f);
	H = 360*sin(t/30.0f)*col + st.col*40 + 180*cos(t/15.0f);
	S = st.k + 100.0f*(carg(z)+PI)/PI;
	//S = 90;
	//if (it == 1)
	//	S = 0;
	//V = 90 + sin(it/5.0f)*40;
...
__kernel void dimension(
	__global uchar4* pix, int xPixels, __global uint* sum)
{

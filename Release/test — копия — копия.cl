	...
	__global uchar4* pix, float4 posData, int xPixels,struct data st, __global float2* c)
{
	...
	{
		if (left)
			x = ((int)idy - right/2)*posData.w*4.0f;
		else
			x = ((int)idy - right/2 - right)*posData.w*4.0f;
		y = ((int)idx - down/2)*posData.w*4.0f;		
	}
	...
	do
	{
		...
		fn1 = Simple(zn, C, c[4], firstFrac(st));
		fn2 = Simple(fn1, C, c[4], secondFrac(st));

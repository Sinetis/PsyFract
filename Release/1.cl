...
float2 Fracs(float2 z, float2 C, __global float2*c, int frac, int k)
{
	...
	case 2:
		return cmul(cpow2(z+fx(c[1].x)),clog(z));
	case 3:
		return cmul(z, cexp(z))*c[1].y + cmul(z, clog(z))*c[2].x;
	case 4:
		return z - (2+c[2].y/10)*cdiv(cpow2(cpow2(z))-fx(1+c[3].x/3.5f),4*cmul(cpow2(z),z) - fx(c[4].x)); 
	case 5:
		return z - (2-c[3].y)*cdiv(ccube(z), 3*cmul(z,z));
	case 6: 
	...
	case 25:
		return cexp(cmul(-z,z+fx(c[0].y)))/sqrt2PI;
	case 26:
	...
	__global uchar4* pix, float4 posData, int xPixels,struct data st, __global float2* c)
{
	...
	do
	{
		...
		//P(A|B) = P(B|A) · P(A) / P(B)
		//z = zn - (1.1 + c[0].x)*cdiv(cmul(fn, zn - zn_1), fn_1 - fn) + C; 
		z = fn + C;
		//z = cpow2(zn - C);

...
float2 Fracs(float2 z, float2 C, __global float2*c, int frac, int k)
{
	...
	case 4:
		return z - (2+c[2].y/10)*cdiv(cpow2(cpow2(z))-fx(1+c[3].x/3.5f),4*cmul(cpow2(z),z) - fx(c[3].x)) + C; 
	case 5:
		return z - (2-c[3].y)*cdiv(ccube(z), 3*cmul(z,z)) + C;
	case 6: 
	...
	__global uchar4* pix, float4 posData, int xPixels,struct data st, __global float2* c)
{
	...
	do
	{
		...
		z = zn - cdiv(cmul(fn, zn - zn_1), fn_1 - fn); 
	...
	int H,S,V;
	H = (col*it*360.0f*(1+c[1].y/3))/st.iter+160-c[2].y*30;
	S = 100; 
	V = 100 + c[0].x*50*0;
	H = H % 360;

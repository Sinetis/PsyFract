...
float2 test2(float2 z, float2 C, float c, int st)
{
	switch(st)
	{
		case 0:
...
float2 test(float2 z, float2 C, float c, int st, float2 (*F)(float2))
{
	return F(z);
	/*switch(st)
	{
	...
	__global uchar4* pix, float4 posData, int xPixels,struct data st, __global float2* c)
{
	...
	{
		//fn_1 = fn;
		//fn = test2(zn, C, j%2 ? c[j/2].x : c[j/2].y, firstFrac(st));
		fn = test2(zn, C, c[0].x, firstFrac(st));
		fn = fn + test2(zn, C, c[1].y, secondFrac(st));
		//fn = fn + test2(zn, C, j%2 ? c[j/2].x : c[j/2].y, firstFrac(st));
		//fn_1 = func(zn_1, C, c, st);
		//fn = Fracs(zn, C, c, firstFrac(st), st.k);
		//fn_1 = Fracs(zn+zn_1, C, c, secondFrac(st), st.k);
		
		//z = zn - (1.1 + c[0].y)*cdiv(cmul(fn, zn - zn_1), fn_1 - fn) + C; 
		z = fn + C; 
